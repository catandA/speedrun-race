...
/* ---------- 1. 语法 ---------- */
if (blocks.length !== 2) { console.error('FAIL: 期望 2 个 script 块, 实际 ' + blocks.length); process.exit(1); }
try { new Function(blocks[0]); new Function(blocks[1]); }
catch (e) { console.error('FAIL syntax: ' + e.message); process.exit(1); }
check(true, '两个 script 块语法正确');
...
(async () => {
  // 错误口令 → 降级选手 + 警告
  let ctx = makeCtx('?room=race1&userId=judge&userSig=SIG&judge=1&judgeKey=wrong');
  vm.runInContext(blocks[1], ctx.sandbox);
  await sleep(200);
  check(!ctx.els.judgePanel && !!ctx.els.playerPanel, '错误口令 → 降级为选手模式');
  check((ctx.els.log.textContent || '').indexOf('裁判口令错误') >= 0, '日志含"裁判口令错误"');

  // 正确口令 → 裁判模式
  ctx = makeCtx('?room=race1&userId=judge&userSig=SIG&judge=1&judgeKey=race2026');
  vm.runInContext(blocks[1], ctx.sandbox);
  await sleep(200);
  ...
  /* ---------- 3. genUserSig 与参考算法一致性 (固定测试密钥) ---------- */
  const fnSrc = blocks[1].match(/async function genUserSig[\s\S]*?\n\}/);
  ...
  const input = `TLS.identifier:player1&TLS.sdkappid:${APP}&TLS.time:${p['TLS.time']}&TLS.expire:${p['TLS.expire']}`;
